...
public enum ColumnType {
	STRING(String.class, new CastService<String>() {
		@Override		public String cast(Object from) {	return from.toString();		}
	}), INTEGER(Integer.class, new CastService<Integer>() {
		@Override		public Integer cast(Object from) {	return Integer.valueOf(from.toString());		}
	}), LONG(Long.class, new CastService<Long>() {
		@Override		public Long cast(Object from) {		return Long.valueOf(from.toString());		}
	}), DOUBLE(Double.class, new CastService<Double>() {
		@Override		public Double cast(Object from) {	return Double.valueOf(from.toString());	}
	}), BOOLEAN(Boolean.class, new CastService<Boolean>() {
		@Override		public Boolean cast(Object from) {	return Boolean.valueOf(from.toString());	}
	}), 
	...
	public static boolean isConvertable(ColumnType from, ColumnType to) {
		...
		if (from == INTEGER && (to == DOUBLE || to == LONG))			return true;
		if (from == LONG && to == DOUBLE)								return true;
		if (to.isList && isConvertable(from.toPlain(), to.toPlain()))	return true;
